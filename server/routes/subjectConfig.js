...
// GET all unique combined batches (active semester)
router.get('/combined-batches/all', auth, async (req, res) => {
  try {
    ...
    // Find all subject allocations for this active semester
    const allocations = await SubjectAllocation.find({ semesterId: activeSem._id })
      .populate('classId');
      
    const combinedBatches = [];
    for (const alloc of allocations) {
      if (alloc.isCombinedBatch && alloc.combinedGroups && alloc.combinedGroups.length > 0) {
        for (const cg of alloc.combinedGroups) {
          if (cg.batches && cg.batches.length > 0) {
            // Label representing the combined batch, e.g. "SE1 - B1 + B2" or similar
            const className = alloc.classId?.name || 'Class';
            const label = `${className} - ${cg.batches.join(' + ')}`;
            if (!combinedBatches.includes(label)) {
              combinedBatches.push(label);
            }
          }
        }
      }
    }
    res.json(combinedBatches.sort());
  } catch (error) {
    res.status(500).json({ message: 'Server error.', error: error.message });
  }
});
