  ...
  return (
    <div className="animate-fadeIn" style={{ maxWidth: 750 }}>
      ...
      <div className="card" style={{ marginBottom: 16, background: 'var(--bg-tertiary)', fontSize: 12 }}>
        <div style={{ fontWeight: 700, marginBottom: 8, color: 'var(--text-muted)' }}>Time Slots Used (Mon–Fri)</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {['8:15-9:15', '9:15-10:15', '10:15-11:15'].map(t => <span key={t} className="badge badge-info">{t}</span>)}
          <span className="badge badge-warning">11:15-11:30</span>
          {['11:30-12:30', '12:30-1:30'].map(t => <span key={t} className="badge badge-info">{t}</span>)}
          <span className="badge badge-warning">1:30-2:30</span>
          {['2:30-3:30', '3:30-4:30', '4:30-5:30'].map(t => <span key={t} className="badge badge-info">{t}</span>)}
        </div>
      </div>
