...
export default function SubjectConfigPanel() {
  ...
  const yearToSem = { 'FE': 2, 'SE': 4, 'TE': 6, 'BE': 8 };
  const currentSem = selectedClassObj?.semester || yearToSem[selectedYear] || '—';
    ...
    <div className="subject-config-page">
      <div className="page-header">
        ...
        <div className="header-filters" style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, fontWeight: 600, color: '#94a3b8' }}>
            SEMESTER
            <span style={{ padding: '6px 12px', background: 'rgba(59,130,246,0.15)', borderRadius: 6, color: '#60a5fa', fontWeight: 700, fontSize: 14 }}>
              Sem {currentSem}
            </span>
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, fontWeight: 600, color: '#94a3b8' }}>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content modal-wide" onClick={e => e.stopPropagation()}>
            ...
            {/* Manual fallback / additional details */}
            <div className="form-grid">
              <div className="form-group">
                <label>Subject Code {availableSubjects.length > 0 ? '(auto-filled)' : ''}</label>
                <input value={form.subject.code} onChange={e => setForm(f => ({ ...f, subject: { ...f.subject, code: e.target.value } }))} placeholder="e.g. CS301" readOnly={availableSubjects.length > 0 && !!form.subject.code} style={availableSubjects.length > 0 && form.subject.code ? { opacity: 0.7, cursor: 'not-allowed' } : {}} />
              </div>
              <div className="form-group">
                <label>Subject Name {availableSubjects.length > 0 ? '(auto-filled)' : ''}</label>
                <input value={form.subject.name} onChange={e => setForm(f => ({ ...f, subject: { ...f.subject, name: e.target.value } }))} placeholder="e.g. Data Structures" readOnly={availableSubjects.length > 0 && !!form.subject.name} style={availableSubjects.length > 0 && form.subject.name ? { opacity: 0.7, cursor: 'not-allowed' } : {}} />
              </div>
              <div className="form-group">
                <label>Type {availableSubjects.length > 0 ? '(auto-filled)' : ''}</label>
