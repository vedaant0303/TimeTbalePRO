...
export default function TimeSlots() {
  ...
  // New: validation and redistribution state
  ...
  useEffect(() => { if (academicYear) fetchSlots(); }, [academicYear]);
  ...
  const fetchSlots = async () => {
    try {
      const res = await api.get(`/timeslots?academicYear=${academicYear}`);
      const data = res.data || [];
  ...
  const handleSeedDefaults = async () => {
    if (!window.confirm('This will DELETE all existing time slots for this academic year and insert the default schedule (8:15 AM – 5:30 PM). Continue?')) return;
    setSeeding(true);
    try {
      await api.post('/timeslots/seed-defaults', { academicYear });
  ...
  const handleAdd = async () => {
    ...
    try {
      if (editId) {
        const res = await api.put(`/timeslots/${editId}`, { ...form, academicYear });
        ...
        await api.post('/timeslots', { ...form, slotNumber: slots.length + 1, academicYear });
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button className="btn btn-primary btn-sm" onClick={() => { setShowForm(!showForm); setEditId(null); setForm({ startTime: '', endTime: '', isBreak: false, breakType: 'none' }); }}>
              {showForm ? '×' : '+ Add Slot'}
            </button>
            <button
              ...
              disabled={seeding}
            >
              {seeding ? '...' : 'Seed Default (8:15–5:30)'}
            </button>
          </div>
        </div>
          ...
          <div style={{ background: 'var(--bg-tertiary)', padding: 16, borderRadius: 'var(--radius-md)', marginBottom: 16, border: '1px solid var(--primary-400)' }}>
            ...
            <div className="grid-2">
              <div className="form-group">
                ...
                <input type="text" className="form-input" placeholder="e.g. 8:15"
                  value={form.startTime} onChange={e => setForm(p => ({ ...p, startTime: e.target.value }))} />
              </div>
              <div className="form-group">
                <label className="form-label">End Time</label>
                <input type="text" className="form-input" placeholder="e.g. 9:15"
        ...
        <div style={{
          ...
          <div style={{ fontWeight: 700, marginBottom: 8, color: '#8b5cf6' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {(() => {
              ...
              for (let i = 0; i < slots.length - 1; i++) {
                if (!slots[i].isBreak && !slots[i + 1].isBreak && slots[i].endTime === slots[i + 1].startTime) {
                  pairs.push({ s1: slots[i], s2: slots[i + 1] });
                }
              }
              return pairs.length > 0 ? pairs.map((p, i) => (
                <span key={i} className="badge" style={{
                  ...
                  {p.s1.startTime}–{p.s2.endTime}
                </span>
              )) : (
                <span style={{ color: '#ef4444', fontWeight: 600 }}>
                  ⚠️ No consecutive pairs found! 2-hour practicals cannot be scheduled.
                </span>
              );
            })()}
          </div>
          <div style={{ marginTop: 8, fontSize: 11, opacity: 0.7 }}>
            Practicals require 2 consecutive slots (back-to-back). Ensure time slots are arranged
            so that one slot's end time matches the next slot's start time (e.g., 8:15-9:15 → 9:15-10:15).
          </div>
        </div>
