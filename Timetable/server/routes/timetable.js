...
router.post('/auto-generate', auth, authorize('coordinator', 'hod'), async (req, res) => {
  try {
    ...
    for (const sem of semestersToGenerate) {
      ...
      // --- FETCH BATCH COMBINE (SubjectAllocation) ENTRIES FOR THIS SEMESTER ---
      let batchCombineEntries = [];
      try {
        const Class = require('../models/Class');
        const classesForSem = await Class.find({ department, semester: sem });
        const classIds = classesForSem.map(c => c._id);
        console.log(`[BatchCombine] Sem ${sem}: Found ${classesForSem.length} classes (${classesForSem.map(c => c.name).join(', ')})`);
        ...
        if (batchCombineEntries.length === 0) {
          ...
          console.log(`[BatchCombine] Fallback query: ${allAllocations.length} total allocations, ${batchCombineEntries.length} for sem ${sem}`);
        }
        
        console.log(`[BatchCombine] Sem ${sem}: ${batchCombineEntries.length} entries → ${batchCombineEntries.map(e => e.subject?.code).join(', ')}`);
      } catch (err) {
        ...
        if (combinedPracticalBlocks.length > 0) {
          ...
          for (const simultaneousBlock of combinedPracticalBlocks) {
            ...
            for (const day of days) {
              ...
              for (const [slot1, slot2] of combConsecutivePairs) {
                if (blockPlaced) break;
  ...
  try {
    ...
    if (semesterId) {
      ...
      const timeSlots = activeSem
        ? await TimeSlot.find({ academicYear: activeSem.academicYear, isActive: true, isBreak: false }).sort({ slotNumber: 1 })
        : [];
    ...
    if (academicYear && department) {
      ...
      for (const e of entries) {
        ...
        if (!grouped[key]) grouped[key] = { subject: e.subject, division: e.division, semester: e.semester, theory: [], practical: [] };
        if (e.type === 'theory' || e.type === 'lecture') {
      ...
      for (const [key, group] of Object.entries(grouped)) {
        ...
        const expectedTheory = sub.theoryHours || 0;
        ...
        // Check practical consecutive slots
        if (group.practical.length >= 2) {
          const byDay = {};
