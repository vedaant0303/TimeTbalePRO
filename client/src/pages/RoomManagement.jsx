...
const emptyForm = { name: '', code: '', building: '', floor: 0, capacity: 30, type: 'classroom', isShared: false, isSharedBatch: false, sharedBatchName: '', facilities: [], department: '', sharedWith: [] };

export default function RoomManagement() {
  const [rooms, setRooms] = useState([]);
  const [departments, setDepartments] = useState([]);
  const [combinedBatches, setCombinedBatches] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const [rRes, dRes, cbRes] = await Promise.all([
        api.get('/rooms'),
        api.get('/departments'),
        api.get('/subject-config/combined-batches/all').catch(() => ({ data: [] }))
      ]);
      setRooms(rRes.data);
      setDepartments(dRes.data);
      setCombinedBatches(cbRes.data || []);
  ...
  const handleEdit = (room) => {
    ...
    setForm({
      ...
      isSharedBatch: room.isSharedBatch || false,
      sharedBatchName: room.sharedBatchName || '',
        ...
        {showForm && (
          <div style={{ background: 'var(--bg-tertiary)', padding: 16, borderRadius: 'var(--radius-md)', marginBottom: 16 }}>
            ...
            <div className="grid-2">
              ...
              <div className="form-group" style={{ display: 'flex', gap: 20, marginTop: 24, alignItems: 'center' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  <input type="checkbox" checked={form.isSharedBatch} onChange={e => setForm(p => ({ ...p, isSharedBatch: e.target.checked, sharedBatchName: e.target.checked ? p.sharedBatchName : '' }))} />
                  <span className="form-label" style={{ margin: 0 }}>Shared Batch</span>
                </label>
              </div>
            </div>
            ...
            {form.isSharedBatch && (
              <div className="form-group" style={{ marginBottom: 16 }}>
                <label className="form-label">Shared Batch (From Batch Combine)</label>
                <select className="form-select" value={form.sharedBatchName || ''} onChange={e => setForm(p => ({ ...p, sharedBatchName: e.target.value }))}>
                  <option value="">— Select Combined Batch —</option>
                  {combinedBatches.map(b => <option key={b} value={b}>{b}</option>)}
                </select>
                {combinedBatches.length === 0 && (
                  <span style={{ fontSize: 11, color: 'var(--text-muted)', display: 'block', marginTop: 4 }}>No combined batches configured in the active semester yet.</span>
                )}
              </div>
            )}
                ...
                <tr key={r._id} style={{ opacity: deleting === r._id ? 0.5 : 1, transition: 'opacity 0.3s' }}>
                  ...
                  <td>
                    {r.isShared && r.sharedWith?.length > 0 && (
                      <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', marginBottom: r.isSharedBatch ? 4 : 0 }}>
                    ...
                    {r.isSharedBatch && r.sharedBatchName && (
                      <span className="badge badge-warning" style={{ fontSize: 9, display: 'inline-block' }}>
                        📦 Batch: {r.sharedBatchName}
                      </span>
                    )}
                    {!r.isShared && !r.isSharedBatch && '—'}
                  </td>
