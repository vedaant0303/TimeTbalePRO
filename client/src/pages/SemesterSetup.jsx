...
export default function SemesterSetup() {
  ...
  const handleActivateSemester = async () => {
    if (!activeConfig) return;
    if (!window.confirm(`Are you sure you want to activate the ${activeConfig.semester} semester (${activeConfig.academicYear})? This will deactivate the current active semester.`)) return;
    try {
      setLoading(true);
      await api.put(`/semester/${activeConfig._id}`, { isActive: true });
      showToast('Semester activated successfully!');
      fetchConfigs();
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to activate semester', 'error');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="semester-setup">
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {activeConfig && !activeConfig.isActive && (
            <button className="btn btn-success" onClick={handleActivateSemester} style={{ background: '#22c55e', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontWeight: 600, cursor: 'pointer' }}>
              Activate Semester
            </button>
          )}
          {activeConfig && (
      ...
      <div className="tab-group" style={{ marginBottom: 24 }}>
        ...
        >
          Odd Semester (1, 3, 5, 7) {configs.find(c => c.semester === 'odd' && c.isActive) && ' (Active 🟢)'}
        </button>
        <button
          className={`tab ${selectedSemType === 'even' ? 'active' : ''}`}
          onClick={() => handleSelectSemType('even')}
        >
          Even Semester (2, 4, 6, 8) {configs.find(c => c.semester === 'even' && c.isActive) && ' (Active 🟢)'}
        </button>
      </div>
