...
export default function MyTimetable() {
  ...
  const [selectedSemType, setSelectedSemType] = useState(() => {
    // Default: Jan-Jun = even, Jul-Dec = odd
  ...
  const handleBrowseView = async () => {
    ...
    // Pick sem based on even/odd selection
    const sem = selectedSemType === 'odd' ? sems[0] : sems[1];
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
              <button
                className={`btn ${selectedSemType === 'even' ? 'btn-primary' : 'btn-secondary'}`}
                style={{ padding: '10px 0', fontWeight: 700 }}
                onClick={() => setSelectedSemType('even')}>
                📗 Even Semester
              </button>
              <button
                className={`btn ${selectedSemType === 'odd' ? 'btn-primary' : 'btn-secondary'}`}
                style={{ padding: '10px 0', fontWeight: 700 }}
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              {['FE', 'SE', 'TE', 'BE'].map(y => {
                const semNum = selectedSemType === 'odd' ? yearSemMap[y][0] : yearSemMap[y][1];
                return (
                  <button key={y}
                    className={`btn ${selectedYear === y ? 'btn-primary' : 'btn-secondary'}`}
                    style={{ padding: '10px 0', fontWeight: 700 }}
                    onClick={() => { setSelectedYear(y); setSelectedSemester(semNum); }}>
                    {y}
                    <div style={{ fontSize: 9, opacity: 0.7, marginTop: 2 }}>Sem {semNum}</div>
                  </button>
                );
              })}
            </div>
      ...
      <div className="flex-between" style={{ marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>
            Division {selectedDiv || user.division} — Semester {selectedSemester || user.semester} ({selectedSemType === 'even' ? 'Even' : 'Odd'}) — Academic Year: {academicYear}
          </p>
        </div>
