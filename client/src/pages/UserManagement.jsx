...
export default function UserManagement() {
  ...
  const filteredUsers = deptFilter
    ? users.filter(u => u.department?._id === deptFilter || u.department === deptFilter)
    : users;
      ...
      <div className="card">
        <div className="card-header">
          <div>
            ...
            <div className="card-subtitle">{filteredUsers.length} user{filteredUsers.length !== 1 ? 's' : ''}{deptFilter ? ' in selected department' : ' registered'}</div>
          </div>
        ...
        <div style={{
          ...
          <span style={{
            ...
            textTransform: 'uppercase', letterSpacing: '0.6px', minWidth: 90
          }}>
            🏛 Department
          </span>
          <button
            ...
            style={{
              ...
              border: !deptFilter ? '2px solid #6366f1' : '2px solid var(--border-color)',
              background: !deptFilter ? 'rgba(99,102,241,0.15)' : 'var(--bg-secondary)',
              color: !deptFilter ? '#6366f1' : 'var(--text-secondary)'
            }}
          >
            All Depts
          </button>
          {departments.map(d => (
            <button
              ...
              style={{
                padding: '5px 14px', borderRadius: 8, fontSize: 12, fontWeight: 700,
                cursor: 'pointer', transition: 'all 0.2s ease',
                border: deptFilter === d._id ? '2px solid #6366f1' : '2px solid var(--border-color)',
                background: deptFilter === d._id ? 'rgba(99,102,241,0.15)' : 'var(--bg-secondary)',
                color: deptFilter === d._id ? '#6366f1' : 'var(--text-secondary)'
              }}
            >
              {d.code || d.name}
            </button>
            ...
            <tbody>
              {filteredUsers.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>
                  No users found{deptFilter ? ' for this department' : ''}.
                </td></tr>
              ) : filteredUsers.map(u => (
